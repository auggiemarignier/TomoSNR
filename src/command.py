...
def process():
    ...
    parser.add_argument('-s','--simscales',nargs='+',default=-1,type=int,help='List of wavelet scales to be simulated, indexed from 0. Integer.')
    ...
    if not par:
        for i,infile in enumerate(infiles,1):
            print(infile)
            run(infile,L=L,B=B,J_min=J_min,simscales=simscales,nmaps=nmaps,tilesize=tilesize,binsave=binsave,save_append=i)
    if par:
        run_par(infiles,L=L,B=B,J_min=J_min,simscales=simscales,nmaps=nmaps,tilesize=tilesize,binsave=binsave)
